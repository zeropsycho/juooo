        ...
        function getLogoAdvert(symbol)
        {
            $.ajax({
                ...
                success:function(result)
                {
                    ...
                    {
                        var advertInfo  = '<div class="logo"><a href="'+result.data.url+'"><img src="picture/c83eca00d6034250a36cc74d4dcb63a7.gif' + result.data.pic + '" alt="'+ result.data.ad_name + '" title="'+ result.data.ad_name + '"></a></div>';
                    }
                    else
                    {
...
<div class="myjuooo w1204">
	<div class="my-menu">
        ...
        <ul>
            ...
            <li><a class=" active " href="mycollect.html"><i></i><b class="arr"></b>我的关注</a></li>
      ...
      <div class="u-title">
          <h3>我的关注</h3>
            <div class="o-t-tips">遇到感兴趣的演出，如果还没决定购买，或暂时缺货，您可以先添加关注，以便下次查找或购买！</div>
        </div>
        <div class="m-collect mt20">
            <div class="collect-op"><label class="op-checkbox"><span class="s-checkbox"></span>全选</label><a href="javascript:void(0);">批量删除</a></div>
          <div class="collect-warp">
					        <div class="no-mth-order">
		           <p class="myfont">&#xe610;</p>
		           <p class="notxt">您还木有关注演出！</p>
            ...
            $.ajax({
                ...
                success:function(result)
                {
                    if(result.code == 1 && result.data)
                    {
                        ...
                        advertInfo += 'src="picture/c83eca00d6034250a36cc74d4dcb63a7.gif' + result.data.pic + '" title="' + result.data.ad_name + '" alt="' + result.data.ad_name + '" >';
                        advertInfo += '</a>';
            ...
            $.ajax({
                ...
                success:function(result)
                {
                    if(result.code == 1 && result.data)
                    {
                        ...
                        html += '<img src="picture/c83eca00d6034250a36cc74d4dcb63a7.gif' + result.data.pic + '" alt="' + result.data.ad_name + '" title="' + result.data.ad_name + '">';
                        html += '</a>';
...
var reloadUrl = "mycollect.html";

seajs.use(['Dialog', 'jquery', 'juo','index'], function(Dialog, $) {
    $(function() {
        var colts = $('.collect-item'),
            checkbox = $('.s-checkbox'),
            collects = new Array(),
            dialog_index;
        if (colts.length == 0) {
            $('.no-mth-order').css('display', 'block');
        }
        checkbox.on('click', function() {
            if ($(this).parent().hasClass('op-checkbox')) {
                (!$(this).hasClass('active')) && $('.s-checkbox').length > 1 ?  checkbox.addClass('active') :checkbox.removeClass('active');
            } else {
                $(this).toggleClass('active');
                if (checkbox.length == $('.collect-item .active').length + 1) {
                    $('.op-checkbox .s-checkbox').addClass('active');
                } else {
                    $('.op-checkbox .s-checkbox').removeClass('active');
                }
            }
        });

        // 批量删除
        $('.collect-op >a').on('click', function() {
        	collects = [];
            var selected = $('.s-checkbox.active').closest('.collect-item');
            //获取选中的删除节点
            $.each(selected, function(key, value){
            	collects[key] = $(value).attr("data-id");
            });
            console.log(typeof(collects));
            if(typeof(collects) != "string")
            {
            	collects = collects.join(',');
            }
            if (selected.length > 0) {
                content = "<div class=\"cal-confirm\" style=\"text-align:center\"><span style=\"display:inline-block;padding:10px 10px 0 0;\">确认要删除关注吗？</span></div>";
                Dialog.confirm({
                    content: content,
                    width: 220,
                    okBtnCallback: function(index){
                        $.post('opendoor', {type: 11, cid: collects}, function(result){
                            if(result.code == 1)
                            {
                                Dialog.close(index);
                                window.location.href=reloadUrl;
                                //$('.active').parent('.collect-item').remove();
                                //colts = $('.collect-item');
                                //checkbox = $('.s-checkbox');
                                //console.log(colts.length);
                                //if (colts.length == 0) {
                                //    $('.no-mth-order').css('display', 'block');
                                //   $('.op-checkbox .s-checkbox').removeClass('active');
                                //}
                            ...
                            {
                            	Dialog.close(index);
                            	content = "<div class=\"cal-confirm\" style=\"text-align:center\"><span style=\"display:inline-block;padding:10px 10px 0 0;\">系统繁忙，请稍后再试</span></div>";
                            	Dialog.alert({
                                    content: content,
                                    width: 220,
                            	})
