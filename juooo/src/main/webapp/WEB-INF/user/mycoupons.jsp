        ...
        function getLogoAdvert(symbol)
        {
            $.ajax({
                ...
                success:function(result)
                {
                    ...
                    {
                        var advertInfo  = '<div class="logo"><a href="'+result.data.url+'"><img src="picture/1811c6db13ff4ee9bab7c073a650082d.gif' + result.data.pic + '" alt="'+ result.data.ad_name + '" title="'+ result.data.ad_name + '"></a></div>';
                    }
                    else
                    {
    ...
    <div class="myjuooo w1204">
        ...
        <ul>
            ...
            <li><a class="" href="myaccount.html"><i></i><b class="arr"></b>账户余额</a></li>
                        ...
                        <li><a class=" active " href="mycoupons.html"><i></i><b class="arr"></b>优惠券</a></li>
            <li><a class="" href="myrecharge.html"><i></i><b class="arr"></b>橙卡</a></li>
            
        </ul>
        <h2 class="my-note"><a href="mynote.html">消息提醒</a></h2>
    </div>        <div class="u-warp2014">
            <div class="u-title clearfix">
                <h3>优惠券</h3>
                <a class="only-link" href="http://www.juooo.com/help/85"  target="_blank">优惠券规则说明»</a>
            </div>
            <div class="note">
                <div class="si-tabs clearfix">
                    <div class="change-coupon">
                        <span class="change-tip"></span>
                        <input class="change-input" type="text" placeholder="输入优惠券码">
                        <a class="change-btn" href="javascript:;">兑换</a>
                    </div>
                    <ul class="si-tabUl">
                        <li class="tab-item active">
                            <a href="javascript:;" data-target="2" data-toggle="tab-list">可使用</a>
                           
                        </li>
                        <li class="tab-item">
                            <a href="javascript:;" data-target="3" data-toggle="tab-list">已使用</a>
                        
                        </li>
                        <li class="tab-item">
                            <a href="javascript:;" data-target="4" data-toggle="tab-list">已过期</a>
                        </li>
                    </ul>
                </div>
                <div class="si-tab-bd">
                    <div class="coupons-wrapper">
                        <!-- 可使用券 -->
                        <div class="tabs-block active" id="tab-box-01">
                
                        </div>
                        <!-- 可使用券 end-->
                        <!-- 已使用券 -->
                        <div class="tabs-block" id="tab-box-02">
                            <div class="coupon-item coupon-used">
                                <div class="top-wrap">
                                    <div class="c-value">￥50</div>
                                    <div class="condition">（满1000可用）</div>
                                    <div class="c-dateline">有效期：2016.07.30-2016.07.30</div>
                                    <span class="c-flag"></span>
                                </div>
                                <div class="bottom-wrap">
                                    <div class="c-name">
                                        <span class="label">券名称：</span>
                                        <span class="name">开学季周末精彩儿童剧</span>
                                        <div class="toggle-down">
                                            <i class="icon-down"></i>
                                            <div class="c-full-name">
                                                开学季周末精彩儿童剧开学季周末精彩儿童剧
                                            </div>
                                        </div>
                                    </div>
                                    <div class="c-order-num">
                                        <span class="label">订单号：</span>
                                        <span class="order-num">2568974253210</span>
                                    </div>
                                    <div class="c-use-time">
                                        <span class="label">使用时间：</span>
                                        <span class="use-time">2016.07.30 12:30</span>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <!-- 已使用券 end -->
                        <!-- 已过期券 -->
                        <div class="tabs-block" id="tab-box-03">
                            <div class="coupon-item coupon-overdue">
                                <div class="top-wrap">
                                    <div class="c-value">￥50</div>
                                    <div class="condition">（满1000可用）</div>
                                    <div class="c-dateline">有效期：2016.07.30-2016.07.30</div>
...
<script>
	...
	(function() {
		...
		ma.async = true;
		ma.src = 'https://click.juooo.com/'+ 'public/Home/web/<%=path%>/resource/user/js/ma.js';
		var s = document.getElementsByTagName('script')[0];
...
//          美洽
            ...
            $.ajax({
                ...
                success:function(result)
                {
                    if(result.code == 1 && result.data)
                    {
                        ...
                        advertInfo += 'src="picture/1811c6db13ff4ee9bab7c073a650082d.gif' + result.data.pic + '" title="' + result.data.ad_name + '" alt="' + result.data.ad_name + '" >';
                        advertInfo += '</a>';
            ...
            $.ajax({
                ...
                success:function(result)
                {
                    if(result.code == 1 && result.data)
                    {
                        ...
                        html += '<img src="picture/1811c6db13ff4ee9bab7c073a650082d.gif' + result.data.pic + '" alt="' + result.data.ad_name + '" title="' + result.data.ad_name + '">';
                        html += '</a>';
    ...
    <script type="text/javascript">
    seajs.use(['jquery', 'Dialog','index'], function($, Dialog) {
        
    	$(function(){
    		var type = $(".si-tabUl").find('.active').find('a').attr('data-target');
    		getMyCouponsList(type);
    	})
    	
        // 使用条件    	
    	$('body').on('click','.js-usage',function(){
    		...
    		$.ajax({
    			type:'post',
    	 		url : 'http://myjuooo.juooo.com/User/couponsInfo',
    	 		data: data,
    	 		dataType:'json',
    	 		success:function(data){
    	 			if(data.code==200)
    	 			{
    	    		 content += "<div class=\"usage-wrapper\">";
    	    		 content +="<ul class=\"content\">";
    	    		 content +=   "<li class=\"list\">1. 单笔订单满"+parseFloat(data.data.limit_low_price)+"元可使用；</li>";
    	    		 content +=   "<li class=\"list\">2. 适用城市："+data.data.limit_city+"；</li>";
    	    		 content +=   "<li class=\"list\">3. 适用演出类型："+data.data.limit_method+"；</li>";
    	    		 content +=   "<li class=\"list\">4. 适用演出类别："+data.data.limit_category+"；</li>";
    	    		 content +=   "<li class=\"list\">5. 限制票价："+data.data.limit_price+"；</li>";
    	             if(data.data.activity_ticket_list.length > 0)
    	             {
	    	    		 content += "<li class=\"list\">6. 仅支持演出：</li>";
	    	             $.each(data.data.activity_ticket_list, function(key, val) {  
	    	                	 content +="<li class=\"list\">排期ID："+data.data.activity_ticket_list[key]['schedular_id']+"　"+data.data.activity_ticket_list[key]['schedular_name']+"</li>";
	    	              });
    	             }
    	    		 content +="</ul>";
    	    		 content +="<div class=\"usage-tip\">(不满足使用条件的优惠券不会展示在结算页)</div>";
    	    		 content +="</div>";
    					Dialog.alert({
    					 title: '使用条件',
    					 content: content,
    	...
    	$('.si-tabUl li').on('click',function(){
    		...
    		{
    			var type = $(this).find('a').attr('data-target');
        ...
        function getMyCouponsList(type)
        {
    		var html     = '';
        	var data = {type:type};
        	$.ajax({
    	 		...
    	 		success:function(data)
    	 		{
    	 			...
    	 			if(data.code==200)
    	 			{
    	 					...
    	 					$.each(data.data.couponCodeList, function(key, val) {  
 								//coupon-used
 								//coupon-overdue
 								var typeClass = '';
 								if(data.data.couponCodeList[key]['coupon_type']==3)
 								{
 									typeClass = 'coupon-used';
 								}
 								if(data.data.couponCodeList[key]['coupon_type']==4)
 								{
 									typeClass = 'coupon-overdue';
 								}
			    	 			html+='<div class="coupon-item '+typeClass+'">';
			    	 			html+='<div class="top-wrap">';
			    	 			html+=' <div class="c-value">￥'+parseFloat(data.data.couponCodeList[key]['coupon_price'])+'</div>';
			    	 			html+='<div class="condition">（满'+parseFloat(data.data.couponCodeList[key]['limit_low_price'])+'可用）</div>';
			    	 			html+='<div class="c-dateline">有效期：'+data.data.couponCodeList[key]['begin_valid_time']+'-'+data.data.couponCodeList[key]['end_valid_time']+'</div>';
			    	 			
			    	 			if(data.data.couponCodeList[key]['coupon_type']==3 || data.data.couponCodeList[key]['coupon_type']==4)
			    	 			{
			    	 				html+='<span class="c-flag"></span>';
			    	 			}
			    	 			
			    	 			html+='</div>';
			    	 			html+='<div class="bottom-wrap">';
			    	 			html+=' <div class="c-name">';
			    	 			html+='<span class="label">券名称：</span>';
			    	 			html+='<span class="name">'+data.data.couponCodeList[key]['coupon_name']+'</span>';
			    	 			html+='<div class="toggle-down">';
			    	 			html+='<i class="icon-down"></i>';
			    	 			html+='<div class="c-full-name">';
			    	 			html+=''+data.data.couponCodeList[key]['coupon_name']+'';
			    	 			html+='</div>';
			    	 			html+='</div>';
			    	 			html+='</div>';
			    	 			
			    	 			if(data.data.couponCodeList[key]['coupon_type']==2 && data.data.couponCodeList[key]['activity_coupon_type']==1)
			    	 			{
			    	 				if(data.data.couponCodeList[key]['isCheck'] == 1)
			    	 				{
			    	 					html+='<a class="use-now"  target="_blank" href="'+searchDomain+'?activity_id='+data.data.couponCodeList[key]['activity_id']+'">立即使用</a>';
			    	 				}
			    	 				else
			    	 				{
			    	 					html+='<a class="use-now" style="width: 145px;"   href="javascript:void(0)">'+data.data.couponCodeList[key]['begin_valid_time']+' 起可用</a>';
			    	 				}
			    	 				html+='<a class="usage js-usage" href="javascript:;" data-cid="'+data.data.couponCodeList[key]['coupon_id']+'">使用条件»</a>';
			    	 			}
			    	 			else if(data.data.couponCodeList[key]['coupon_type']==3)
			    	 			{
			    	 				
			    	 				html+='<div class="c-order-num">';
			    	 				html+='<span class="label">订单号：</span>';
			    	 				html+='<span class="order-num">'+data.data.couponCodeList[key]['order_sn']+'</span>';
			    	 				html+='</div>';
			    	 				html+='<div class="c-use-time">';
			    	 				html+='<span class="label">使用时间：</span>';
			    	 				html+='<span class="use-time">'+data.data.couponCodeList[key]['used']+'</span>';
			    	 				html+='</div>';
			    	 				
			    	 			}
			    	 			else if(data.data.couponCodeList[key]['coupon_type']==4)
			    	 			{
        ...
        $(".change-btn").on('click',function(){
        	var couponCode = $('.change-input').val();
        	if(couponCode=='')
        	{
        		$('.change-tip').html('请输入优惠券码！');
        		return false;
        	}
        	var data = {couponCode:couponCode}
        	$.ajax({
    	 		type:'post',
    	 		url : 'http://myjuooo.juooo.com/User/handleCouponCode',
    	 		data: data,
    	 		dataType:'json',
    	 		success:function(data)
    	 		{
    	 			...
    	 			if(data.code==200)
    	 			{
    	 				$('.change-tip').html('兑换成功');
    	 				location.reload();
    	 			}
    	 			else
    	 			{
    	 				$('.change-tip').html(data.msg);
    	 			}
