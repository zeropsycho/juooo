        ...
        function getLogoAdvert(symbol)
        {
            $.ajax({
                ...
                success:function(result)
                {
                    ...
                    {
                        var advertInfo  = '<div class="logo"><a href="'+result.data.url+'"><img src="picture/6de1247124e84a449836f9e47d9da395.gif' + result.data.pic + '" alt="'+ result.data.ad_name + '" title="'+ result.data.ad_name + '"></a></div>';
                    }
                    else
                    {
...
<div class="myjuooo w1204">
    <div class="my-menu">
        ...
        <ul>
            <li><a class=" active " href="myorderlist.html"><i></i><b class="arr"></b>我的订单</a></li>
        ...
        <div class="u-title">
            <h3>我的订单</h3>
            <div class="o-t-tip">
                <a href="myorderlist.html"   class="all-order" >全部订单</a>
                <a href="myorderlist.html/status/1/"    >待支付<em class="num"></em></a>
                <a href="myorderlist.html/status/2/" >待收货<em class="num"></em></a>
                <a href="myorderlist.html/status/3/" >待评价<em class="num"></em></a>
            </div>
        </div>
            ...
            $.ajax({
                ...
                success:function(result)
                {
                    if(result.code == 1 && result.data)
                    {
                        ...
                        advertInfo += 'src="picture/6de1247124e84a449836f9e47d9da395.gif' + result.data.pic + '" title="' + result.data.ad_name + '" alt="' + result.data.ad_name + '" >';
                        advertInfo += '</a>';
            ...
            $.ajax({
                ...
                success:function(result)
                {
                    if(result.code == 1 && result.data)
                    {
                        ...
                        html += '<img src="picture/6de1247124e84a449836f9e47d9da395.gif' + result.data.pic + '" alt="' + result.data.ad_name + '" title="' + result.data.ad_name + '">';
                        html += '</a>';
...
seajs.use(['juo', 'jquery', 'Dialog', 'countDown','index'], function(ex, $, Dialog) {
	getOrderStatusList(1);
	
    $('.moi-op .time').countdown(function($obj) {
        var orderSn = $obj.closest('.moi-op').find('.order-cancel').attr('data-sn');
        $.post(doorUrl, {
            type: 8,
            orderSn: orderSn,
            clearCon: '自动删除'
    ...
    $(function() {
        var baobeis = $('.mo-item .baobei');
        baobeis.each(function() {
            if ($(this).siblings('.baobei').length == 0) {
                $(this).parents('.moi-right').addClass('only-baobei');
                var height1 = $(this).parents('.moi-right').height();
                var height2 = $(this).height();
                $(this).parents('.moi-prod').css('margin-top', (height1 - height2) / 2 + 'px');
                //luotao修改
                $(this).parents('.moi-right').find('.remain-pay').addClass('margin-top13');
                //注释
                //$(this).parents('.moi-right').find('.remain-pay').css('margin-top', (height1 - height2) / 2 + 'px');
            }
        });
    });

    var flag = false;
    $('.remind-shipping').on('click', function() {
        if(flag) return;
        flag = true;
        var remindContent;
        var remindOrderSn = $(this).attr('data-sn');
        ...
        $.ajax({
            ...
            data: {
                type: 13,
                orderSn: remindOrderSn
            },
            dataType: 'json',
            async: false,
            success: function(result) {
                // 请求成功后
                remindContent = result.data;
            },
            error: function(result) {
                // 请求错误
                remindContent = '系统繁忙，请稍后再试'
            },
        });

        var lay_index = Dialog.msg({
            content: remindContent,
            seconds: 1
        });
        setTimeout(function(){
            Dialog.close(" + lay_index + ");
            flag = false;
        }, 1000);
    });
    ...
    var expressFlag;
    // 查看物流
    $('.logistics-detail,.modd-parcel').on('mouseover', function() {
        var list = '';
        var listClass, length;
        var invoiceNo = $('.logistics-detail,.modd-parcel').find('a').attr('data-invoice');
        var obj = $(this).parents('.mo-item').find('.modd-parcel');
        if (obj.find('ul').find('li').length == 0) {
            expressFlag = 0;
            $.ajax({
                type: 'POST',
                url: doorUrl,
                ...
                success: function(result) {
                    // 请求成功后
                    if (result.code == 1 && typeof(result['data']) != "string") {
                        length = result['data'].length;
                        $.each(result.data, function(key, item) {
                            listClass = (key == 0) ? 'latest' : 'done';
                            list += '<li class="' + listClass + '">' + item.operate_message + ' ' + item.operate_time + '</li>';
                        });
        ...
        if (expressFlag == 1) {
            $(this).parents('.mo-item').find('.modd-parcel').css('display', 'block');
        }
    });

    $('.logistics-detail,.modd-parcel').on('mouseout', function() {
        $(this).parents('.mo-item').find('.modd-parcel').css('display', 'none');
    });

    $(function(){
        $(document).on('click','.del-order',function(){
            var content = '<div style="text-align: center;"><span>你确定要删除此订单吗？</span></div>',
                order_sn = $(this).closest('.mo-head').find('.order-sn').text();

            Dialog.confirm({
                content: content,
                width: 450,
                okBtnCallback: function(index){
                    updateOrderStatus(1,order_sn);
                    Dialog.close(index);
                }
            })
        })
