        ...
        function getLogoAdvert(symbol)
        {
            $.ajax({
                ...
                success:function(result)
                {
                    ...
                    {
                        var advertInfo  = '<div class="logo"><a href="'+result.data.url+'"><img src="picture/f88a558f4f384c4ba2bcde37a7b6c7ad.gif' + result.data.pic + '" alt="'+ result.data.ad_name + '" title="'+ result.data.ad_name + '"></a></div>';
                    }
                    else
                    {
...
<div class="my-menu">
        ...
        <ul>
            ...
            <li><a class=" active " href="myrecharge.html"><i></i><b class="arr"></b>橙卡</a></li>
    ...
    <div class="u-warp2014 mysecurity">
        <div class="u-title clearfix">
            <h3><!-- <i class="myfont">&#xe61b;</i> -->我的橙卡</h3>
            <div class="o-t-tips">为保障帐户和余额安全，建议进行<a href="mysecurity.html">安全设置</a></div>
        </div>
        <div class="g-myjuooo mt20">
            <div class="mt-avatar">
                <dl class="clearfix">
                    <dt><a href="myprofile.html"><img src="picture/logo.jpg"><p class="up">上传头像</p></a></dt>
                    <dd class="userinfo">
                        <p class="name">hua</p>
                        <div class="user-lv">等级：普通用户<a href="http://www.juooo.com/help/58">了解会员等级»</a></div>
                    </dd>
                </dl>
            </div>
            <div class="mt-integral mt-changePassword">
                <a href="javascript:;" class="binding-card">
                    <i class="myfont">&#xe60b;</i>
                    <p>[ 绑定橙卡 ]</p>
                </a>
            </div>
            <div class="mt-integral mt-information">
                <a href="http://myjuooo.juooo.com/User/rechargemoney">
                    <i class="myfont ico-mt-money">&#xe611;</i>
                    <p>[ 查询余额 ]</p>
                </a>
            </div>
            <div class="mt-secure">
...
<div class="recharge-num">目前账户可用橙卡0张</div>
<div class="stockPor myrecharge">
    <div class="st-tit">
        <ul class="clearfix">
            <li class="st-th1">橙卡卡号<b class="line"></b></li>
            <li class="st-th2">面值<b class="line"></b></li>
            <li class="st-th3">余额<b class="line"></b></li>
            <li class="st-th4">有效期<b class="line"></b></li>
        </ul>
    </div>
    <table class="recharge-table" width="100%" border="0" cellspacing="0" cellpadding="0">
        <colgroup>
...
<div class="ui-faq">
    <dl class="faq-explain">
        <dt>使用说明：</dt>
        <dd>
1.此卡可购买聚橙网演出票<br/>
2.此卡不记名，不挂失，请妥善保管卡号和密码<br/>
3.使用前，须将此卡绑定到聚橙网注册账号中，一经绑定，不得解绑<br/>
4.此卡不兑换现金，不设找赎，购票使用不开具发票<br/>
5.次卡有效期自购卡日期两年，请在有效期内使用，过期作废<br/>
    ...
    <dl class="faq-tips">
        <dt>温馨提示：</dt>
        <dd>
       可以联系聚橙客服购买，客服电话：<b>400-185-8666</b>。<br><br><br>
如何使用橙卡？<br>
<a target="_blank" href="http://www.juooo.com/help/69" class="charge-more">点击这里，得到详情</a>
</dd>
    </dl>
</div>
</div>
            ...
            $.ajax({
                ...
                success:function(result)
                {
                    if(result.code == 1 && result.data)
                    {
                        ...
                        advertInfo += 'src="picture/f88a558f4f384c4ba2bcde37a7b6c7ad.gif' + result.data.pic + '" title="' + result.data.ad_name + '" alt="' + result.data.ad_name + '" >';
                        advertInfo += '</a>';
            ...
            $.ajax({
                ...
                success:function(result)
                {
                    if(result.code == 1 && result.data)
                    {
                        ...
                        html += '<img src="picture/f88a558f4f384c4ba2bcde37a7b6c7ad.gif' + result.data.pic + '" alt="' + result.data.ad_name + '" title="' + result.data.ad_name + '">';
                        html += '</a>';
...
</html><div id="orderPopCon" class="g-mask" style="display:none;" charset="utf-8">
    <div id="g_popWarp" class="g-popWarp">
        <div class="g-popBg g-p5 pop-text pop-text-310">
            <div class="g-popBgFff">
                <div class="p-text-box">
                    <i class="ico-warn"></i>您需要先<a href="mysecurity.html" style="color:blue">开通支付密码</a>后才能绑定橙卡！
                </div>
                <!-- <div class="p-text-btn">
                    <a class="btn" href="mysecurity.html" style="background:#ff7039">确定<b class="arrBg"></b></a>
                    <a class="btn" onclick="closealert()" style="color:#000;border:1px solid #ccc">取消<b class="arrBg"></b></a>
                </div> -->
            </div>
        </div>
    </div>
...
<script>
	seajs.use(['Dialog','juo'], function(Dialog) {
	    $(function(){
	        var content;
	        $('.binding-card').on('click',function(){
	            $.ajax({
	                type:'POST',
	                url:'http://myjuooo.juooo.com/User/checkpaypass',
	                success:function(data){
	                    if(data==1){
	                        window.location.href='http://myjuooo.juooo.com/User/rechargebind';
	                    }else{
	                        content = $("#orderPopCon").html();
	                        Dialog.confirm({
	                            content: content,
	                            width:220,
	                            okBtnCallback: function(index){
	                                window.location = 'mysecurity.html';
	                                Dialog.close(index);
	                            }
	                        })
	                    }
...
</script><script type="text/javascript" src="<%=path%>/resource/user/js/jquery.min.js"></script>
...
function cardaction(id){
	$("#cardform"+id).attr('action','http://myjuooo.juooo.com/User/rechargeinfo')
	$("#cardform"+id).submit();
}
